...
const PersonalInformation = () => {
    ...
    const areAllInputFilled = () => {
        if (getValues("gender") !== '' && getValues("maritalStatus") !== '' && getValues("nationalId") !== '' && getValues("dateOfBirth") !== '' && getValues("copyOfNationalId")) {
            setValid(true);
        } else {
            setValid(false)
        }
    ...
    const handleImageFiles = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (e.target.files) {
            uploadToFirebase(e.target.files[0] ?? null)
                .then((uploaded) => {
                    setValue("copyOfNationalId", uploaded);
                    areAllInputFilled();
                })
                .catch((error) => {
                    console.log(error);
                        ...
                        render={() => (
                            <FormItem>
                                <FormLabel>
                                    ...
                                    {getValues("copyOfNationalId") && <span className="text-sm text-green-400">Uploaded (Do not upload again)</span>}
                                </FormLabel>
                                <FormControl>
                                    <Input
                                        type="file"
